...
<form class="layui-form" method="post"  >
    ...
    <div class="layui-form-item">
        ...
        <div class="layui-input-block">
            <input type="radio" name="isStart" value="0" title="是" [#if clubActivity??][#if clubActivity.isStart==0]checked[/#if] [#else ]checked[/#if] >
            <input type="radio" name="isStart" value="1" title="否" [#if clubActivity??][#if clubActivity.isStart==1]checked[/#if] [/#if]>
        </div>
    </div>
   ...
   [#-- <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <select name="clubId" lay-filter="selFilter">
                [#list clubs as club]
                    <option value="${club.id}" [#if clubActivity.club.id=club.id]selected="selected" [/#if] >${club.name}</option>
                [/#list]
            </select>
        </div>
    </div>--]
...
<script type="text/javascript">
    layui.use(["form",'layedit','laydate','upload'],function () {
        ...
        form.on('submit(demo1)', function(data){
            ...
            var clubId=$("select[name='clubId']").val();
            var content=layedit.getContent(editIndex);
            $.ajax({
                url:"${base}/admin/club_activity/save.jhtml",
                data:{id:id,"club.id":clubId,title:name,content:content,isStart:isStart,endTime:date,images:images},
                type:"post",
                dataType:"json",
