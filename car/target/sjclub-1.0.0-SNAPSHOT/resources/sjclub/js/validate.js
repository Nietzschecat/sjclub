...
$(document).ready(function(){
	...
	/*log_pas_input.blur(function(){
		if(log_acc_input.val() == ""){
			//不能为空
			showErrorTip("账号不能为空！",log_acc,log_acc_gly,log_acc_tip);
		}else{
				//检查账号是否存在
				$.get("/user/checkAccount.jhtml?account="+log_acc_input.val()+"&"+new Date().getTime(),
					function(data){
						//邮箱不存在则提示信息
						if(data == "notExist"){
							showErrorTip("此账号不存在！",log_acc,log_acc_gly,log_acc_tip);
						}else if(data == "ok"){
	...
	log_sub.click(function(){
		...
		console.log(log_pas_input.val());
		$.ajax({
			url:"/admin/login.jhtml",
			data:{username:log_acc_input.val(),password:log_pas_input.val()},
			type:"post",
			dataType:"json",
			success:function(data){
				if(data.code == "pasIsError"){
					//密码错误则提示
					showErrorTip("密码错误，请输入正确密码！",log_pas,log_pas_gly,log_pas_tip);
				}else if(data.code == "ok"){
					//密码正确则刷新本页
					window.location.href="/sjclub/index.jhtml";
	...
	reg_name_input.focus(function(){
		if(reg_acc_input.val() == ""){
			//不能为空
			showErrorTip("账号不能为空！",reg_acc,reg_acc_gly,reg_acc_tip);
		}else{
				//检查账号是否存在
				$.get("/user/checkAccount.jhtml?account="+reg_acc_input.val()+"&"+new Date().getTime(),
					function(data){
						if(data == "accExist"){
							showErrorTip("此账号已存在，请重新输入！",reg_acc,reg_acc_gly,reg_acc_tip);
						}else if(data == "ok"){
	...
	reg_pas_input.focus(function(){
		...
		if(reg_name_input.val() == ""){
			...
			/*$.get("../Register?action=checkName&name="+reg_name_input.val()+"&"+new Date().getTime(),
				function(data){
					if(data == "nameExist"){
						showErrorTip("此用户已存在，请重新输入！",reg_name,reg_name_gly,reg_name_tip);
					}else if(data == "ok"){
	...
	reg_rep_input.focus(function(){
		if(reg_acc_input.val() == ""){
			//不能为空
			showErrorTip("登录账号不能为空！",reg_acc,reg_acc_gly,reg_acc_tip)
		}
		if(reg_name_input.val() == ""){
			//不能为空
			showErrorTip("用户姓名不能为空！",reg_name,reg_name_gly,reg_name_tip);
		}
	...
	reg_sub.click(function(){
		...
		if(reg_name_input.val() == ""){
			//不能为空
			showErrorTip("姓名不能为空！",reg_name,reg_name_gly,reg_name_tip);
		}
		if(reg_pas_input.val() == ""){
			//不能为空
			showErrorTip("密码不能为空！",reg_pas,reg_pas_gly,reg_pas_tip);
			...
			showErrorTip("请输入密码！",reg_rep,reg_rep_gly,reg_rep_tip);
			 ...
			 if(reg_rep_input.val() != reg_pas_input.val())
				 ...
				 $.ajax({
					 ...
					 data:{userName:reg_name_input.val(),loginName:reg_acc_input.val(),password:reg_pas_input.val(),sex:sex},
					 type:"post",
					 success:function(data){
						  if(data == "ok"){
							 //密码正确则刷新本页
